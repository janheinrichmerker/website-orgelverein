...

LTIwMjAwODI1AQIDBAUGBwgJ
-----END OPENSSH PRIVATE KEY-----"
host="orgel-braunschweigerdom.de"
path="/web/pages"
...
function sftp-command() {
  echo "$@" | sftp -b - -i "$temp/key" "$user@$host"
}

function sftp-test-connection() {
  sftp-command "exit" >/dev/null 2>&1
}

function sftp-full-list() {
  sftp-command "ls -fl $1" 2>&1 |
    sed -n '/sftp>/,$p' |
    tail -n +2
}
